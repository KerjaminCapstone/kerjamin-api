...
func ListFreelance(c echo.Context) error {
	...
	err := db.Raw(`SELECT fd.date_join, case when cd.is_male = true then 'Pria' else 'Wanita' end as jenis_kelamin, fd.id_freelance,u."name" , fd.rating , fd.profile_pict , fd.job_done , fd.points , fd.is_trainee,fd.is_male , jcc.job_child_name ,(6371 * acos( cos( radians(fd.address_lat) ) * cos( radians( ? ) ) *cos( radians( ? ) - radians(fd.address_long) ) 
